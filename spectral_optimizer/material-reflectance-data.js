/*
 * Seven representative material reflectance models used by the material
 * colour-comparison workbench. Appearance rendering and spectral evidence are
 * tracked independently. The built-in curves are engineering references unless
 * explicitly replaced by user-measured data.
 */
(function (root, factory) {
    ...
    const MATERIAL_ATLAS = 'assets/material-texture-atlas.png';
    ...
    function material(definition, index) {
        const spectralSource = Object.freeze({
            type: 'engineering',
            label: '工程参考曲线',
            notes: definition.notesCN || '按典型材料反射特征构建。',
            dataQualification: DATA_QUALIFICATION
        });
        const appearanceSource = Object.freeze({
            type: 'photo',
            label: '原始材质渲染图集',
            origin: 'local seven-material renderer atlas',
            file: MATERIAL_ATLAS,
            atlasPosition: Object.freeze([index, 0]),
            atlasGrid: Object.freeze([7, 1]),
            baseFilter: 'none',
            notes: '同一格材质纹理用于参考光与当前光谱对比；综合色由计算结果叠加。'
        });
        const anchors = Object.freeze(definition.anchors.map(function (pair) { return Object.freeze(pair.slice()); }));
        return Object.freeze({
            ...
            sourceName: spectralSource.label,
            sourceUrl: '',
            sourceSample: '',
    ...
    const definitions = [
        {
            id: 'wood_warm_oak', name: 'Warm oak wood', nameCN: '暖橡木饰面', category: 'wood',
            targetHueZone: 'red-orange-yellow',
            intendedUse: 'wood, veneer and warm interior finish optimisation',
        ...
        {
            id: 'wood_dark_walnut', name: 'Dark walnut wood', nameCN: '深胡桃木饰面', category: 'wood',
        ...
        {
            id: 'leather_cognac', name: 'Cognac leather', nameCN: '干邑色皮革', category: 'leather',
            targetHueZone: 'red-orange-brown',
            intendedUse: 'leather, warm upholstery and hospitality material optimisation',
        ...
        {
            id: 'fabric_warm_beige', name: 'Warm beige fabric', nameCN: '暖米色织物', category: 'fabric',
        ...
        {
            id: 'leaf_green', name: 'Green leaf', nameCN: '绿植叶片', category: 'plant',
            targetHueZone: 'green',
        ...
        {
            id: 'skin_tone_sample', name: 'Skin-tone reference sample', nameCN: '肤色参考样本', category: 'skin-tone-sample',
            targetHueZone: 'orange-red',
            intendedUse: 'human-centred scene rendering simulation',
            intendedUseCN: '用于以人为本场景的肤色呈现模拟；涉及产品声明时应采用实测数据。',
            anchors: [[380,0.18],[400,0.22],[450,0.30],[500,0.38],[550,0.45],[600,0.55],[650,0.62],[700,0.66],[780,0.68]]
        },
        {
            id: 'neutral_wall_matte', name: 'Neutral matte wall', nameCN: '中性哑光墙面', category: 'neutral',
    ...
    const materials = Object.freeze(definitions.map(material));
    const byId = Object.freeze(materials.reduce(function (map, item) {
        map[item.id] = item;
        return map;
    }, {}));

    function getMaterial(id) { return byId[id] || null; }
