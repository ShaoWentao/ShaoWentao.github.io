...
(function (root) {
    ...
    // Replace the legacy parser above with a strict parser. The old parser
    // silently guessed 1 nm spacing for every one-column file and clamped bad
    // reflectance values, which could produce a plausible but wrong material.
    function parseReflectanceCSV(text) {
        if (!text || typeof text !== 'string') return { error: 'file is empty' };
        var lines = text.split(/\r?\n/).filter(function (line) {
            return line.trim() && !isHeaderLine(line);
        });
        if (lines.length < 5) return { error: 'at least 5 numeric rows are required' };

        var hasWL = likelyHasWavelengthColumn(lines);
        var rows = lines.map(function (line) {
            return line.split(/[,\t\s]+/).map(Number).filter(function (value) { return Number.isFinite(value); });
        ...
        if (hasWL) {
            ...
            var step = rows.length === 401 ? 1 : rows.length === 81 ? 5 : 0;
            if (!step) return { error: 'single-column files must contain exactly 81 (5 nm) or 401 (1 nm) values' };
            rows.forEach(function (row, index) {
                if (row.length >= 1) pairs.push([WL_START + index * step, row[0]]);
            });
        }
        if (pairs.length < 5) return { error: 'not enough wavelength/reflectance pairs' };
        pairs.sort(function (a, b) { return a[0] - b[0]; });
        for (var index = 1; index < pairs.length; index++) {
            if (pairs[index][0] === pairs[index - 1][0]) return { error: 'duplicate wavelengths are not allowed' };
        }
        if (pairs[0][0] > 380 || pairs[pairs.length - 1][0] < 780) {
            return { error: 'wavelength coverage must include 380-780 nm' };
        }
        var maxValue = pairs.reduce(function (max, pair) { return Math.max(max, pair[1]); }, 0);
        if (maxValue > 1.5) {
            pairs = pairs.map(function (pair) { return [pair[0], pair[1] / 100]; });
        }
        if (pairs.some(function (pair) { return pair[1] < 0 || pair[1] > 1; })) {
            return { error: 'reflectance values must be between 0 and 1 or 0 and 100 percent' };
        }
        var values = interpolateToGrid(pairs);
        if (!values || values.length !== WAVELENGTH_COUNT) {
            return { error: 'interpolation failed for the 380-780 nm grid' };
        }
