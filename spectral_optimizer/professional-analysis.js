(function (root) {
    ...
    var alphaData = null;
    var latest = null;
    var CHROMATICITY_DIAGRAM = root.ChromaticityDiagram || {};
    var selectedCurves = new Set();
    var contributionMetric = 'photopic';
    var channelPointsVisible = false;
    ...
    function loadOfficialData() {
        var localFileMode = root.location && root.location.protocol === 'file:';
        if (localFileMode) {
            var compatibilityStatus = document.getElementById('professional-data-status');
            if (compatibilityStatus) {
                compatibilityStatus.textContent = 'CIE S 026 数据未加载；完整功能请运行 start-local.bat。';
            }
            if (root.SpectralAppReadiness) {
                root.SpectralAppReadiness.markFailed('professional', '专业数据需要通过本地 HTTP 服务加载。');
            }
            return Promise.resolve();
        }

        var loadPromise = root.ProfessionalDataLoader &&
            typeof root.ProfessionalDataLoader.loadProfessionalDataText === 'function'
            ? root.ProfessionalDataLoader.loadProfessionalDataText({
                fetchImpl: typeof root.fetch === 'function' ? root.fetch.bind(root) : null,
                url: 'cie-alpha-opic-action-spectra.csv'
            })
            : Promise.reject(new Error('Professional data loader unavailable'));
        ...
        return loadPromise
            .then(function (text) {
                ...
                if (status) status.textContent = '作用光谱：CIE S 026:2018 官方 1 nm 数据（已加载）。';
                document.dispatchEvent(new CustomEvent('spectral-professional-curves-ready'));
                if (root.SpectralAppReadiness) {
                    root.SpectralAppReadiness.markReady('professional', '专业数据加载完成。');
                }
                if (latest) render(latest);
            })
            .catch(function (error) {
                console.error('CIE S 026 data loading failed:', error);
                var status = document.getElementById('professional-data-status');
                if (status) status.textContent = 'CIE S 026 数据加载失败，请刷新页面。';
                if (root.SpectralAppReadiness) {
                    root.SpectralAppReadiness.markFailed('professional', '专业数据加载失败，已进入兼容模式。');
                }
            });
    }
    ...
    function injectUi() {
        ...
        if (controls && !document.getElementById('action-curve-menu')) {
            ...
            details.innerHTML =
                ...
                Object.keys(curveMeta).map(function (key) {
                    var value = key === 'photopic'
                        ? '<em class="action-curve-reference">基准</em>'
                        : '<strong data-action-curve-der="' + key + '">--</strong>';
                    return '<label data-action-curve="' + key + '"><input type="checkbox" value="' + key + '">' +
                        '<span class="action-curve-swatch" style="--curve-color:' + curveMeta[key].color + '"></span>' +
                        '<span class="action-curve-label">' + curveMeta[key].label + '</span>' + value + '</label>';
                }).join('') +
                '<small>数值为当前光谱相对于同照度 D65 的 DER；曲线仅作归一化参考叠加。</small>' +
                '<small class="professional-data-status" id="professional-data-status">正在加载 CIE S 026 作用光谱…</small></div>';
        ...
        var contributionSelect = document.getElementById('channel-contribution-metric');
        if (contributionSelect && !contributionSelect.dataset.professionalBound) {
            contributionSelect.dataset.professionalBound = 'true';
            contributionSelect.addEventListener('change', function (event) {
    ...
    function xyToProfessional(x, y) {
        var uv = CHROMATICITY_DIAGRAM.xyTo1960Uv(x, y);
        var upvp = CHROMATICITY_DIAGRAM.xyTo1976UpVp(x, y);
        return {
            u: uv.u,
            v: uv.v,
            up: upvp.up,
    ...
    function drawCoordinateDiagram(canvas, payload, mode) {
        ...
        var xMax = 0.60;
        var yMax = 0.60;
        var geometry = CHROMATICITY_DIAGRAM.createPlotGeometry(width, height, {
            xMax: xMax,
            yMax: yMax,
            padding: padding
        });
        var plotWidth = geometry.plotWidth;
        var plotHeight = geometry.plotHeight;
        function px(value) { return CHROMATICITY_DIAGRAM.projectPoint(value, 0, geometry).x; }
        function py(value) { return CHROMATICITY_DIAGRAM.projectPoint(0, value, geometry).y; }
        ...
        for (var tick = 0; tick <= 0.6; tick += 0.1) {
            ...
            ctx.beginPath();
            ctx.moveTo(px(tick), geometry.top);
            ctx.lineTo(px(tick), geometry.bottom);
            ctx.stroke();
            ctx.beginPath();
            ctx.moveTo(geometry.left, py(tick));
            ctx.lineTo(geometry.right, py(tick));
            ctx.stroke();
            ctx.textAlign = 'center';
            ctx.fillText(tick.toFixed(1), px(tick), geometry.bottom + 17);
            ctx.textAlign = 'right';
            ctx.fillText(tick.toFixed(1), geometry.left - 7, py(tick) + 3);
        }

        var locus = [];
        for (var wavelength = 380; wavelength <= 780; wavelength++) {
        ...
        if (locus.length) {
            var cacheKey = mode + ':' + Math.round(width) + 'x' + Math.round(height) + '@' + dpr;
            if (!canvas._chromaticityBackground || canvas._chromaticityBackground.key !== cacheKey) {
                ...
                // Each UCS sample is converted back to xy before rendering.
                var sampleStep = 0.006;
                for (var coordinateX = 0; coordinateX <= xMax; coordinateX += sampleStep) {
                    for (var coordinateY = 0; coordinateY <= yMax; coordinateY += sampleStep) {
                        var sampleXy = mode === '1960'
                            ? CHROMATICITY_DIAGRAM.uv1960ToXy(coordinateX, coordinateY)
                            : CHROMATICITY_DIAGRAM.upvp1976ToXy(coordinateX, coordinateY);
                        var sampleX = sampleXy.x;
                        var sampleY = sampleXy.y;
                        ...
                        offCtx.fillRect(
                            px(coordinateX),
                            py(coordinateY) - geometry.scale * sampleStep,
                            geometry.scale * sampleStep + 1,
                            geometry.scale * sampleStep + 1
                        );
                    }
                }
                canvas._chromaticityBackground = { key: cacheKey, canvas: offscreen };
            }
            ctx.drawImage(canvas._chromaticityBackground.canvas, 0, 0, width, height);
            ...
            for (var gridTick = 0; gridTick <= 0.6; gridTick += 0.1) {
                ...
                ctx.beginPath();
                ctx.moveTo(px(gridTick), geometry.top);
                ctx.lineTo(px(gridTick), geometry.bottom);
                ctx.stroke();
                ctx.beginPath();
                ctx.moveTo(geometry.left, py(gridTick));
                ctx.lineTo(geometry.right, py(gridTick));
                ctx.stroke();
            }
        }
        ...
        // ticks in the selected UCS coordinate system.
        var planckian = [];
        if (root.SpectralMath && typeof root.SpectralMath.planckianXy === 'function') {
            for (var temperature = 1600; temperature <= 12000; temperature += 50) {
                ...
                var planckPoint = transformXy(planckXy.x, planckXy.y, mode);
        ...
        // Channel points are optional because the default research view focuses on
        // the current, target and Planckian-locus relationship.
        if (channelPointsVisible) {
        var gamutPoints = (payload.channels || []).filter(function (channel) {
            return Number.isFinite(channel.x) && Number.isFinite(channel.y);
        }).map(function (channel) {
            var transformed = transformXy(channel.x, channel.y, mode);
        ...
        if (payload.target && Number.isFinite(payload.target.x) && Number.isFinite(payload.target.y)) {
            var target = transformXy(payload.target.x, payload.target.y, mode);
            ctx.save();
            ctx.strokeStyle = '#ff6b25';
            ctx.lineWidth = 1.6;
            ctx.setLineDash([3, 2]);
            ctx.beginPath();
            ctx.arc(px(target.x), py(target.y), 6, 0, Math.PI * 2);
            ctx.stroke();
            ctx.setLineDash([]);
            ctx.fillStyle = '#b34d19';
            ctx.textAlign = 'left';
            ctx.fillText('目标', px(target.x) + 8, py(target.y) - 6);
            ctx.restore();
        }

        if (payload.baseline && Number.isFinite(payload.baseline.x) && Number.isFinite(payload.baseline.y)) {
            var baseline = transformXy(payload.baseline.x, payload.baseline.y, mode);
            ctx.save();
            ctx.strokeStyle = 'rgba(85, 82, 77, 0.78)';
            ctx.lineWidth = 1.3;
            ctx.setLineDash([3, 3]);
            ctx.beginPath();
            ctx.moveTo(px(baseline.x), py(baseline.y));
            ctx.lineTo(px(current.x), py(current.y));
            ctx.stroke();
            ctx.beginPath();
            ctx.arc(px(baseline.x), py(baseline.y), 6, 0, Math.PI * 2);
            ctx.stroke();
            ctx.setLineDash([]);
            ctx.fillStyle = '#5f5a53';
            ctx.textAlign = 'left';
            ctx.fillText('基准', px(baseline.x) + 8, py(baseline.y) + 4);
            ctx.restore();
        }
        ...
        ctx.textAlign = 'right';
        ctx.fillText(mode === '1960' ? 'u' : 'u′', geometry.right, geometry.bottom + 28);
        ctx.textAlign = 'left';
        ctx.fillText(mode === '1960' ? 'v' : 'v′', geometry.left - 24, geometry.top + 4);
    }

    function renderDiagrams(payload) {
        document.documentElement.dataset.cie1976ChannelPoints = channelPointsVisible ? 'visible' : 'hidden';
        drawCoordinateDiagram(document.getElementById('professional-cie1976'), payload, '1976');
    }

    function renderAlphaOpic(payload) {
        if (!alphaData || !payload.fallback || !payload.fallback.d65) return;
        ...
        keys.forEach(function (key) {
            ...
            var output = document.querySelector('[data-action-curve-der="' + key + '"]');
            if (output) output.textContent = number(der, 2);
        });
    }
    ...
    function renderDeferred(payload) {
        latest = payload;
        var dominant = dominantWavelengthAndPurity(payload.x, payload.y, payload.fallback);
        var xyz = payload.xyz || {};
        var xyzEl = document.getElementById('professional-xyz');
        var dominantEl = document.getElementById('professional-dominant');
        var purityEl = document.getElementById('professional-purity');
        if (xyzEl) xyzEl.textContent = 'X ' + number(xyz.X, 3) + ' · Y ' + number(xyz.Y, 3) + ' · Z ' + number(xyz.Z, 3);
        if (dominantEl) dominantEl.textContent = Number.isFinite(dominant.wavelength) ? '主波长 ' + dominant.wavelength + ' nm' : '主波长 --';
        if (purityEl) purityEl.textContent = Number.isFinite(dominant.purity) ? '色纯度 ' + Math.min(100, dominant.purity).toFixed(1) + '%' : '色纯度 --';
    ...
    function setChannelPointsVisible(visible) {
        channelPointsVisible = Boolean(visible);
        document.documentElement.dataset.cie1976ChannelPoints = channelPointsVisible ? 'visible' : 'hidden';
        refresh();
    }

    function refresh() {
        if (latest) renderBase(latest);
    }

    function init() {
        document.documentElement.dataset.cie1976ChannelPoints = 'hidden';
        injectUi();
        loadOfficialData();
    }
    ...
    root.SpectralProfessional = {
        init: init,
        update: render,
        updateBase: renderBase,
        updateDeferred: renderDeferred,
        refresh: refresh,
        setChannelPointsVisible: setChannelPointsVisible,
